/*
    TODO:
          ...
          ekran ölçeği oyun sürecinde değişince tıklanan yer algılanması güncellenecek
          yol türü değişimleri oluşturucuda olmalı ve cost hesabında kullanılmalı
          algoritma seçimi butonla sağlanacak
          engeller yalnızca asfaltta ve kaygan yolda olmalı
...
const PERSPECTIVE = [0.5,
  0.5]; // Binalar varsayılan olarak ortadan bakan birinin göreceği şekilde 3d çiziliyor, başka oyunlarda yine kuş bakışı olmasına rağmen yukarıdan veya aşağıdan bakılmış gibi çizenler olmuş, belirtilen değerler sırasıyla genişlik ve yüksekliğe göre ölçekleniyor
...
const ROAD_CONDITION_WEIGHTS = {"asphalt":0.8,"dirt":0.2,"slippery":0}
...
let imagesArray = ["temp_car.png", "ocean.jpeg", "bina_test.png", "bina_yan.png", "park alanı.jpg", "cim.jpg", ...
  ROAD_IMAGES, ...OBSTACLE_IMAGES, ...LIGHT_IMAGES,
];
...
let getNextDirection = (roadType, angle, fromDirection, possibleDirections,carAngle) => {
  ...
  return roadType == "4" ? getOpposite(fromDirection) : roadType == "3" ? angleLookup[angle] == fromDirection ?
    possibleDirections.find(e => e != fromDirection) : getOpposite(fromDirection) : roadType == "rightcurve" ?
    !isNaN(carAngle)?possibleDirections.find(e=>e==angleLookup[carAngle])||possibleDirections.find(e=>e!=fromDirection):possibleDirections.find((e) => e != fromDirection): possibleDirections.find(e => e != fromDirection)
};
...
let createMap = (grid, curr, fromDirection) => {
  ...
  let currRoads = firstInsert ? ["straight"] : ROAD_TYPES_ARR.slice(0).sort((x, y) => currWeights[x] - currWeights[
    y]);
...
let findSuitableRoad = (point, connections, grid) => {
  ...
  if (connections.length == 2) {
    ...
    let difference = connectionArray[(indexLeft + 3) % 4] == directions[1] || connectionArray[(indexRight + 3) % 4] ==
      directions[0] ? 1 : 2;
...
let fixRoad = (points, grid) => {
  ...
  points.forEach((connectionGroup) => {
    connectionGroup.forEach((currPoint) => {
      ...
      let newConnections = getConnections(ROAD_TYPES_ARR[e[0]], e[1]).map((e) => currNeighbours[
        connectionLookup[e]]).filter((e) => {
...
let pruneRoads = (grid) => {
  ...
  grid.forEach((e, i) => e.forEach((e, j) => {
    ...
    let notCompatible = found && currentConnections.find(
      (e) => found[1] != e[1] && inBounds(e[1]) && grid[e[1][0]][e[1][1]][0] != threeIndex && grid[e[1][0]][e[
        1][1]][0] != fourIndex);
...
let getCost = (roadTypeNumber,amount)=>{
  ...
  switch(roadType){
    case "straight":
      return Math.max(5,10-amount*1.5)
    case "rightcurve":
      return Math.min(15,10+amount*1.5)
    default:
      return 7.5
  }
}
let getCosts = (grid,curr=getStartPoint(grid),visitedObj,consecutiveRoadType=-1,consecutiveCoords=[])=>{
    ...
    let roadTypeNumber = grid[currX][currY][0]
    ...
    let nextCost = getCost(consecutiveRoadType,consecutiveAmount)
    //son değer zaten aşağıda atanacak
    for(let i = 0;i<consecutiveAmount-1;i++){
      ...
      if(consecutiveAmount>currCounter){
        visitedObj[e[0]][e[1]]=[consecutiveAmount,nextCost]
      }
    }
    if(!isSameRoad){
    ...
    visitedObj[currX][currY]=[1,getCost(consecutiveRoadType,consecutiveCoords.length)]
    let left = grid[currX][currY];
    if (left[0] == -1) return false;
    let leftNeighbours = getNeighbours(curr);
    let directionsAndConnections = getConnections(ROAD_TYPES_ARR[left[0]], left[1]).map((e) => [e, leftNeighbours[
      connectionLookup[e]]]);
...
let findPath = (grid, pathAlgorithm, road1Indexes, road2Indexes, getMinimumDistance = false, forceInitialDirection,
  visited, visitedObj, lastDirection = forceInitialDirection) => {
  switch (pathAlgorithm) {
    case "A*": {
      ...
      while (openSet.length > 0) {
        ...
        let connections = getConnections(ROAD_TYPES_ARR[current[0]], current[1]).map((e) => neighbours[
          connectionLookup[e]]);
    ...
    case "ucs": {
      ...
      while (queue.length > 0) {
        ...
        let leftConnections = getConnections(ROAD_TYPES_ARR[left[0]], left[1]).map((e) => leftNeighbours[
          connectionLookup[e]]);
        if (isInitial && forceInitialDirection) {
    ...
    default: {
      ...
      let forcedDirection = forceInitialDirection ? forcedIsArray ? forceInitialDirection.map(
        (e) => leftConnections[connectionLookup[e]]) : leftNeighbours[connectionLookup[forceInitialDirection]] :
        null;
      if (isInitial && forceInitialDirection) {
        if (DIRECTION_ALTERNATIVE == 1) {
          if (leftConnections.includes(forcedDirection)) {
      ...
      for (let i = 0; i < leftConnections.length; i++) {
        ...
        let tempRes = findPath(grid, null, curr, road2Indexes, getMinimumDistance, null, visited.map((e) => e),
          copyVisitedObj(visitedObj), direction);
        if (tempRes) {
...
let findPathTo = function (x, y, getMinimumDistance, forceInitialDirection) {
  ...
  let res = findPath(this.game.map, this.pathAlgorithm, currIndexes, gridIndexes, getMinimumDistance,
...
let isOnLineSegment = (P, Q, R) => {
  return (Q[0] < Math.max(P[0], R[0]) && Q[0] > Math.min(P[0], R[0]) && Q[1] < Math.max(P[1], R[1]) && Q[1] > Math
    .min(P[1], R[1]));
};
...
class Entity {
  ...
  getColliders() {
    ...
    return entities.filter((e) => {
      ...
      if (e == this || e.entityType == "sensor" || (this.entityType != "sensor" && cachedGrids.isDisjointFrom(
        currGrids))) return false;
  ...
  init(sprite) {
    ...
    if (this.createGraphics) {
      ...
      this.boundingRect = this.scaledBounds.map(
        (e, i) => e - (i == 0 ? this.sprite.width * this.anchorX : i == 1 ? this.sprite.height * this.anchorY : 0));
      this.drawGraphics();
    }
  }
  a = 0;
  ...
  tick(dt) {
    ...
    if (this.isAutonomous) {
      ...
      if (this.lastAction) {
        ...
        if (updateAction) this.lastAction = this.getAction
      }
    }
...
export class Car extends MovableEntity {
  ...
  setWanderGoal(){
    ...
    console.log("goal!",this.posX,this.posY,[chosenRoad.posX,chosenRoad.posY])
  ...
  setGoal(x, y) {
    ...
    let nextDirection = currRoadType == "4" || currRoadType == "3" || currRoadType == "rightcurve" ? getNextDirection(
      currRoadType, currRoad.direction, fromDirection,null,this.getRoundedDirection()) : currentDirection;
  ...
  tick(dt) {
    ...
    if (this.isRecording) {
      if (this.goal) {
        ...
        let relativeDirection = getRelativeDirection(this.path.length > 1 ? this.path[0] : this.gridIndexes,
          relativePathCoords);
        ...
        let input = [...this.sensors.map((e) => e.output[0] / this.width), ...relativeGoal, ...relativePath, ...
          velocity,
        ];
        let output = [
  ...
  steer(dt, angle) {
    ...
    if (Math.abs(alignment) < ((this.entityMinAlignment / this.absoluteVel()) * this.entityMoveMultiplier) / ((isGoingBackwards ? this.entityTurnLimiters[0] :
      this.entityTurnLimiters[1]) + (this.isUsingBrake ? 2 : 0))) return;
    this.isTurning=true
...
export class Road extends Entity {
  ...
  getLines() {
    ...
    let mapped = res.map((e, i) => e.map((e, j) => e.map((e, q) => e * (1 - RATIO) + res[(i + 2) % 4][+!j][q] *
      RATIO)));
    let retVal = [];
  ...
  getHighlightCoordinates(index) {
    let currentAngle = connectionLookup[getConnections(this.roadType, this._direction)[index]] * 90 -
...
export class Building extends Filler {
  ...
  setPosition(x, y) {
    ...
    let skewTop = -Math.atan2(leftSpace / 2 - Math.min(absOffsetX, absOffsetY) + this.spriteWidth, this.spriteWidth) *
      Math.sign(offsetX) * Math.sign(offsetY);
    let skewLeft = Math.atan2(leftSpace / 2 - Math.min(absOffsetY, absOffsetX) + this.spriteWidth, this.spriteWidth) *
      Math.sign(offsetX) * Math.sign(offsetY);
    if (offsetY > 0) {
...
function calculateVehicleProperties(roadCondition, isTurning = false,isBraking=false) {
  ...
  return {acceleration,drag,turnDrag,steering,turnLimiters,alignment};
}
...
let getEdge = (grid) => {
  return grid.map((e, i) => e.map((e, i) => [e, i]).filter((e) => e[0][0] == -1 && (i == 0 || i == GRID_WIDTH - 1 ||
    e[1] == 0 || e[1] == GRID_HEIGHT - 1)).map((w) => {
      return [
...
export class Game {
  ...
  setRoads() {
    let roads = [];
    let lastCondition = "asphalt"
    let changePossibility = 1/ROAD_CONDITION_WEIGHTS[lastCondition]/8
    for (let i = 0; i < GRID_WIDTH; i++) {
      ...
      for (let j = 0; j < GRID_HEIGHT; j++) {
        ...
        let tempRoad = new Road(this, ROAD_TYPES_ARR[curr[0]], 0, curr[1],lastCondition);
        let rand = Math.random()
        if(rand<changePossibility){
          lastCondition=getWeightedRandom(ROAD_CONDITION_WEIGHTS)
          changePossibility = 1/ROAD_CONDITION_WEIGHTS[lastCondition]/8
        }
        roads[i][j] = tempRoad;
    ...
    this.map.forEach((e, i) => e.forEach((e, j) => {
      ...
      if (e[0] == -1 && !filled[key]) {
        ...
        let possibleFillers = fillers.filter(
          (e) => fillersCounters[e][1] == -1 || fillersCounters[e][0] < fillersCounters[e][1]);
