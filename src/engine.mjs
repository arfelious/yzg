...
let getNextDirection = (
  ...
  return roadType == '4'
    ...
    : roadType == 'rightcurve'
    ? !isNaN(carAngle)
      ? possibleDirections.find((e) => e == angleLookup[carAngle]) ||
        possibleDirections.find((e) => e != fromDirection)
      : possibleDirections.find((e) => e != fromDirection)
    : possibleDirections.find((e) => e != fromDirection);
};
...
let getCosts = (
  grid,
  curr = getStartPoint(grid),
  visitedObj,
  consecutiveRoadType = -1,
  consecutiveCoords = []
) => {
  ...
  visitedObj[currX][currY] = [
    1,
    getCost(consecutiveRoadType, consecutiveCoords.length),
  ];
    ...
    case 'ucs': {
      ...
      while (queue.length > 0) {
        ...
        for (let next of leftConnections) {
          ...
          let roadType = grid[nextX][nextY][0];
          let baseCost =
            roadType !== -1 ? costs[nextX][nextY]?.[1] || 10 : Infinity;
...
export class Car extends MovableEntity {
  ...
  setWanderGoal() {
    let chosenRoad;
    if (!this.currentRoad) {
      chosenRoad = this.game.possibleRoads
        .map((e) => this.game.roads[e[0]][e[1]])
        .map((e) => [e, getMagnitude(this.posX - e.posX, this.posY - e.posY)])
        .sort((x, y) => x[1] - y[1])[1][0];
      console.log('00', chosenRoad);
    } else {
      let opposite = getOpposite(this.getFacingDirection());
      let indexes = getConnections(
        this.currentRoad.roadType,
        this.currentRoad._direction
      )
        .filter((e) => e != opposite)
        .map((e) => getSubgridIndex(connectionLookup[e]));
      chosenRoad = shuffle(
        indexes
          .map((e) => inBounds(e) && this.game.roads[e[0]]?.[e[1]])
          .filter((e) => e)
      )[0];
      if (!chosenRoad) {
        let chosenGoal = getCoordinates(
          this.currentRoad.gridIndexes[0] + indexes[0][0],
          this.currentRoad.gridIndexes[1] + indexes[0][1]
        );
  ...
  _getGoalAction(dt) {
    ...
    let willTurn =
      this.path.length > 1 &&
      getRelativeDirection(this.path[0], this.path[1]) ==
  ...
  setVehicleProperties() {
    let currentProperties = calculateVehicleProperties(
      this.currentRoad?.roadCondition || 'asphalt',
      this.isTurning,
      this.isUsingBrake
    );
    let { acceleration, drag, turnDrag, steering, turnLimiters, alignment } =
      currentProperties;
  ...
  steer(dt, angle) {
    ...
    if (
      Math.abs(alignment) <
      ((this.entityMinAlignment / this.absoluteVel()) *
        this.entityMoveMultiplier) /
        ((isGoingBackwards
          ? this.entityTurnLimiters[0]
          : this.entityTurnLimiters[1]) +
          (this.isUsingBrake ? 2 : 0))
    )
      return;
    this.isTurning = true;
...
function calculateVehicleProperties(
  roadCondition,
  isTurning = false,
  isBraking = false
) {
...
export class Game {
  ...
  createWanderer() {
    ...
    wanderer.direction =
      connectionLookup[
        shuffle(getConnections(road.roadType, road._direction))[0]
      ] * 90;
